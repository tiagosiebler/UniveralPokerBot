...
- (NSImage*)getSubImageWithRect:(NSRect)subRect{
    ...
    @autoreleasepool {
        ...
        croppedNSImage = [[NSImage alloc] initWithCGImage:croppedCGImage size:NSMakeSize(subRect.size.width, subRect.size.height)];
        [croppedNSImage makeThreadSafe];
...
- (NSImage*)getBWSubImageWithRect:(NSRect)subRect{
    NSImage *result = [self getSubImageWithRect:subRect];
    [result makeThreadSafe];

    result = [CVHelper greyScaleImage:result];
    return result;
}
- (NSImage*)greyScaleImage{
    NSImage *result = nil;
    result = [CVHelper greyScaleImage:self];
    [result makeThreadSafe];
    return result;
}
- (NSImage*)clearForBets{
    return [CVHelper applyBlindsThresholdImage:self];
}
- (NSImage*)clearForBlinds{
    return [CVHelper applyTableChipsThresholdImage:self];
}
- (NSImage*)clearForPots{
    return [CVHelper applyPotsThresholdImage:self];
}
